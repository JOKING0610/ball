/* ============================================================
 * app.js —— 交互层(表情球站点外壳,只依赖 EmotionBall SDK + EB_I18N)
 *
 * 职责:
...
(function () {
  ...
  var prefs = { lang: 'zh', theme: 'dark', mode: 'wall', shape: 'blob', sketch: false, tourMs: 2500, speed: 'fast' };
  try {
  ...
  /* 切换速度档位:流畅(默认,过渡缩短+封顶,跟手) / 标准(保留表情原始时长) / 轻柔(更慢更拖) */
  var SPEED_PRESETS = {
    fast:     { scale: 0.6, max: 500 },
    standard: { scale: 1,   max: 0 },
    gentle:   { scale: 1.4, max: 1500 }
  };
  ...
  function createMain(shape) {
    ...
    var sp = SPEED_PRESETS[prefs.speed] || SPEED_PRESETS.fast;
    main = EB.create(elStage, {
      emotion: emotion,
      shape: shape || 'blob',
      transitionScale: sp.scale,
      transitionMax: sp.max,
  ...
  elSpeedSelect.addEventListener('change', function () {
    prefs.speed = elSpeedSelect.value;
    savePrefs();
    var sp = SPEED_PRESETS[prefs.speed] || SPEED_PRESETS.fast;
    main.setTransitionSpeed(sp.scale, sp.max);
    var key = { fast: 'speedFast', standard: 'speedStandard', gentle: 'speedGentle' }[prefs.speed];
    toast(I.t('toastSpeed', { name: I.t(key || prefs.speed) }), 'ok');
  });
  ...
  elSpeedSelect.value = prefs.speed;
