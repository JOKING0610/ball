...
(function () {
  ...
  /* 表情切换用:开头立刻动起来,前 50% 完成 87.5%,快速到达目标 */
  ...
  function Engine(target, opts) {
    ...
    /* ---- 切换速度:过渡时长缩放 + 封顶 + 连点检测(默认保持 SDK 原行为) ---- */
    this._transitionScale = opts.transitionScale || 1;
    this._transitionMax = opts.transitionMax || 0;
    /* 手动切换(点击/翻页/AI 下发,非 auto)额外提速,让操作即时反馈 */
    this._manualScale = opts.manualScale != null ? opts.manualScale : 0.5;
  ...
  Engine.prototype = {
    ...
    setEmotion: function (id, o) {
      ...
      /* 切换速度:时长缩放 → 封顶 → 手动切换(点击/翻页等,非 auto)再压一档 →
       * 连点(距上次切换 <350ms 视为快速翻页)再减半。
       * 自动切换(巡演/待机/序列跳转)保持档位节奏,手动操作即时反馈 */
      var dur = this._prevPose ? def.transition : 0;
      dur *= this._transitionScale;
      if (this._transitionMax > 0 && dur > this._transitionMax) dur = this._transitionMax;
      if (!o.auto) {
        dur *= this._manualScale;
        if (this._transitionMax > 0 && dur > this._transitionMax * this._manualScale) {
          dur = this._transitionMax * this._manualScale;
        }
      }
      if (now - this._lastSwitchAt < 350) dur *= 0.5;
      ...
      /* 状态切换：眼环弹到新池首个表情（自动切换 14/11，手动切换更快 18/14），
       * 并且切换瞬间先眨一次眼（睡眠 / 停止类除外） */
      this._poolPos = 0;
      var springW = o.auto ? (def.poolSpeed >= 10 ? 14 : 11) : (def.poolSpeed >= 10 ? 18 : 14);
      this._setExpr(def.pool[0], springW);
    ...
    /* 运行时调整切换速度:scale 缩放过渡时长,max 封顶(0 = 不限) */
    setTransitionSpeed: function (scale, max) {
      this._transitionScale = scale || 1;
      this._transitionMax = max || 0;
      return this;
    },
    ...
    _compose: function (now, depth) {
      ...
      /* 表情切换过渡插值(easeOutCubic:起步即动,快速收敛,跟手) */
      var tt = now - this._transStart;
      if (this._transDur > 0 && tt < this._transDur && this._prevPose) {
        pose = lerpPose(this._prevPose, pose, easeOutCubic(tt / this._transDur));
      }
      return pose;
    },
